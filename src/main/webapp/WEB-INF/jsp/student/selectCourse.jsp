				...
				<table class="table table-bordered">
					...
					<%--	<c:if test="${!item.over}">--%>
							<tr>
								...
								<td>${item.course.coursetime}</td>
								<td>${item.course.classroom}</td>
								<td>${item.course.courseweek}</td>
								<td>${item.course.coursetype}</td>
								<td>${item.course.score}</td>
								<td>
									<button class="btn btn-default btn-xs btn-info" onClick="location.href='/student/outCourse?id=${item.course.courseid}'">退课</button>
									<!--弹出框-->
								</td>
							</tr>
						<%--</c:if>--%>
